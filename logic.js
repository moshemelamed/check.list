...
class List extends React.Component {
    ...
    render() {
        return (
            <div className="todo" >
                {this.props.activities.map((activity, i) => <div key={i} className="list" onClick={this.handeleCheck}>
                <input type="checkbox" ></input><span>{i + 1}. </span >{this.generateActivityString(activity)} <img id={i} className="delete" src="delete-512.png" onClick={this.deletFromList}></img></div>)}
            </div>
        );
    }
}
            ...
            <div >
                ...
                <div className="lists">
                    ...
                    <div className="second_list">
                        ...
                        <List activities={this.state.done} deletFromList={this.deletForGood} backToList={this.handleSubmit}/>
                    </div>
                </div>
            </div>
        );
